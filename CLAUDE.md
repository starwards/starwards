...
| `room.send()` values arrive as garbage (e.g. 0.25 → -1.58e-309) | @colyseus/schema@3.0.76 sensitivity: adding `@gameField` fields to **two** schema classes in the same build corrupts command decoding (either class alone is fine; reproducible with inert dummy fields). Not root-caused. Avoid growing two synced classes at once; keep server-only data off the synced schema |
