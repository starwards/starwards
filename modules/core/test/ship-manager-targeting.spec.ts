...
describe('ShipManager housekeeping', () => {
    ...
    it('ammo counts are clamped when magazine capacity drops', () => {
        ...
        expect(magazine.count_CannonHe).to.equal(magazine.design.max_CannonHe);
        magazine.capacity = 0.5;
        runTick(mgr);
        expect(magazine.count_CannonHe).to.equal(magazine.max_CannonHe);
        expect(magazine.count_CannonHe).to.be.lessThan(magazine.design.max_CannonHe);
    });
