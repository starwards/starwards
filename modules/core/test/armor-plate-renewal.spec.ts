...
describe('armorPlateRenewal (docked-tier repair-queue protocol)', () => {
    ...
    it('B3: undocking mid-run cancels only the running renewal; already-renewed plates keep their health', () => {
        const { state, repairManager } = setUpShip();
        state.docking.mode = DockingMode.DOCKED;
        state.armor.plateRepairSeconds = 1;
        ...
        expect(slotFor(state, 'armorPlateRenewal').priority).to.equal(RepairPriority.OFF);

        raise(state, 'armorPlateRenewal'); // no auto-repeat — must be re-toggled per plate
        tick(repairManager, 0.1); // promotes 2nd run to RUNNING
        expect(slotFor(state, 'armorPlateRenewal').priority).to.equal(RepairPriority.RUNNING);
    ...
    it('B4: a fully stripped hull is fully restored within the ruled 2-3 minute window when re-raised after each completion', () => {
        const { state, repairManager } = setUpShip();
        state.docking.mode = DockingMode.DOCKED;
        for (const plate of state.armor.armorPlates) {
        ...
        for (let i = 0; i < plateCount; i++) {
            raise(state, 'armorPlateRenewal');
            const perRunSeconds = state.armor.plateRepairSeconds + 0.1;
            runTicks(repairManager, perRunSeconds, 20);
            totalSeconds += perRunSeconds;
            expect(slotFor(state, 'armorPlateRenewal').priority).to.equal(RepairPriority.OFF);
        }
    ...
    it('B5: plate health never exceeds max, and progress does not advance at game speed 0', () => {
        ...
        const progressBefore = slotFor(state, 'armorPlateRenewal').progress;

        repairManager.update({ deltaSeconds: 0, deltaSecondsAvg: 0, totalSeconds: 0 });

        expect(slotFor(state, 'armorPlateRenewal').progress).to.equal(progressBefore);
    ...
    it('damage races repair: combat damage taken while a run is active is not undone by that run completing', () => {
        ...
        raise(state, 'armorPlateRenewal');
        tick(repairManager, 0.5); // running, partway through
