 ...
 * invulnerable). HiExp penetrates only internal systems and most of a dragonfly's are external,
 * so a kill needs the doctrine's switch to Frag once the facing plates are gone.
 */
function runPinnedDragonflyAttack() {
    ...
    let fragFromAt: number | null = null;
    ...
    for (const id of makeIterationsData(SIM_SECONDS, SIM_SECONDS * HZ)) {
        ...
        if (fragFromAt === null && gun.projectile === 'FragShell') {
            fragFromAt = id.totalSeconds;
        }
    }
    return { armorStrippedAt, fragFromAt, killedAt, maxBroken, systems: targetMgr.state.systems().length };
}

describe('NPC ammo doctrine against a dragonfly-MK1', () => {
    jest.setTimeout(120_000);

    it('fires HiExp until the facing plates are stripped, then Frag, and kills the target', () => {
        const result = runPinnedDragonflyAttack();
        const summary = JSON.stringify(result);

        expect(result.fragFromAt, summary).to.not.equal(null);
        // Frag only once the facing plates are broken -- never before the armor is stripped.
        expect(result.armorStrippedAt, summary).to.not.equal(null);
        expect(result.fragFromAt!, summary).to.be.at.least(result.armorStrippedAt! - 1);
        expect(result.maxBroken, summary).to.be.at.least(7);
