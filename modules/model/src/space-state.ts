...
export class SpaceState extends Schema {
  ...
  public static clientInit(clientState: SpaceState) {
    ([
      ...
      'registerOnRemove',
      Symbol.iterator
    ] as Array<keyof SpaceState>).forEach(
      p => ((clientState[p] as any) = SpaceState.prototype[p])
    );
    clientState.events = new EventEmitter();
    clientState.asteroids.onAdd = clientState.spaceships.onAdd = (
      so: SpaceObject
    ) => clientState.events.emit('add', so);

    clientState.asteroids.onRemove = clientState.spaceships.onRemove = (
      so: SpaceObject
    ...
    clientState.events.on(
      'add',
      (so: SpaceObject) =>
        (so.onChange = changes => clientState.events.emit(so.id, changes))
    );
  }
