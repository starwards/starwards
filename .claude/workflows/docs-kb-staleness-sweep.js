export const meta = {
    ...
    description:
        'Audit the docs/ knowledge base for claims that contradict the current code, verify each finding adversarially, and (optionally) apply surgical corrections',
    whenToUse:
        'Run to refresh the documentation KB after code changes — finds doc claims (paths, APIs, versions, commands, formulas, status) that no longer match the codebase. Pass args to scope it or to preview without editing.',
    phases: [
        ...
        {
            title: 'Fix',
            detail: 'apply confirmed corrections, then read back to measure what landed (skipped in dry-run)',
            model: 'opus',
        },
    ],
};
...
const DISCOVERY = {
    ...
    properties: {
        discoveredFileCount: {
            type: 'integer',
            description:
                'total count of non-excluded Markdown files you enumerated under the root; must equal the sum of files across all units (a coverage check)',
        },
        units: {
            type: 'array',
            ...
            items: {
                ...
                properties: {
                    ...
                    depth: {
                        ...
                        enum: ['deep', 'light'],
                        description:
                            'deep = technical doc whose claims map to code; light = design/historical/planning doc',
                    },
                    files: {
                        type: 'array',
                        items: { type: 'string' },
                        description: 'repo-relative doc paths in this unit (1-5 files)',
                    },
                },
            },
        },
        ...
        findings: {
            ...
            items: {
                ...
                properties: {
                    ...
                    quote: {
                        type: 'string',
                        description: 'short verbatim excerpt from the doc containing the stale claim',
                    },
                    claim: { type: 'string', description: 'what the doc asserts' },
                    evidence: {
                        type: 'string',
                        description: 'specific code file/lines or command output proving it is stale today',
                    },
            ...
            items: {
                ...
                properties: {
                    ...
                    applied: {
                        type: 'boolean',
                        description:
                            'true only if the correction is now present in the file AND the stale text is gone',
                    },
...
const discovered = await agent(discoveryPrompt(), {
    label: 'discover',
    phase: 'Discover',
    schema: DISCOVERY,
    model: 'sonnet',
...
if (reportedCount && reportedCount > filesAudited) {
    log(
        `⚠ coverage gap: discovery reported ${reportedCount} non-excluded files but only ${filesAudited} were grouped — ${reportedCount - filesAudited} doc(s) may be unaudited.`,
    );
}
...
log(
    `Discovered ${UNITS.length} units, ${filesAudited} files (${UNITS.filter((u) => u.depth === 'deep').length} deep, ${UNITS.filter((u) => u.depth === 'light').length} light)`,
);
...
const results = await pipeline(
    UNITS,
    (unit) =>
        agent(auditPrompt(unit), { label: `audit:${unit.key}`, phase: 'Audit', schema: FINDINGS, model: 'sonnet' }),
    async (audit, unit) => {
        if (!audit || !audit.findings.length) return { unit: unit.key, findings: 0, confirmed: [] };
        if (budgetLow()) {
            log(
                `verify:${unit.key} → skipped (token budget low); ${audit.findings.length} candidate(s) left unverified`,
            );
            ...
            audit.findings.map(
                (f) => () =>
                    agent(verifyPrompt(f), {
                        label: `verify:${unit.key}:${f.file.split('/').pop()}`,
                        phase: 'Verify',
                        schema: VERDICT,
                        model: 'opus',
                    }).then((v) => ({ f, v })),
        ...
        const confirmed = verdicts
            .filter((x) => x && x.v && x.v.isStale)
            .map((x) => ({
                ...x.f,
                correction: x.v.revisedCorrection || x.f.correction,
                verifierReasoning: x.v.reasoning,
            }));
        return { unit: unit.key, findings: audit.findings.length, confirmed };
    },
    async (res) => {
        ...
        if (budgetLow()) {
            log(
                `fix:${res.unit} → skipped (token budget low); ${res.confirmed.length} confirmed correction(s) NOT applied`,
            );
        ...
        const fixed = await agent(fixPrompt(res.confirmed), {
            label: `fix:${res.unit}`,
            phase: 'Fix',
            schema: FIXED,
            model: 'opus',
        ...
        const readback = await agent(verifyAppliedPrompt(res.unit, res.confirmed), {
            label: `readback:${res.unit}`,
            phase: 'Fix',
            schema: APPLIED,
            model: 'haiku',
        });
        const measured =
            readback && Array.isArray(readback.results) ? readback.results.filter((r) => r && r.applied).length : null;
        if (measured !== null && measured !== claimedEdits.length) {
            log(
                `readback:${res.unit} → fixer claimed ${claimedEdits.length} edit(s), ${measured} verified present in files`,
            );
        }
        return {
            ...res,
...
const summary = {
    ...
    editsApplied: ok.reduce(
        (n, r) => n + (typeof r.editsVerified === 'number' ? r.editsVerified : r.edits ? r.edits.length : 0),
        0,
    ),
    editsClaimed: ok.reduce((n, r) => n + (r.edits ? r.edits.length : 0), 0),
    perUnit: ok
        .filter((r) => r.confirmed && r.confirmed.length)
        .map((r) => ({
            unit: r.unit,
...
log(
    `Done: ${summary.candidateFindings} candidates, ${summary.confirmedFindings} confirmed, ${summary.editsApplied} edit(s) verified-applied${APPLY ? ` (${summary.editsClaimed} claimed by fixer)` : ' (dry-run, none applied)'}`,
);
return summary;
