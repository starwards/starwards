...
**`energyStarved`:** set by `EnergyManager.drawEnergy` (`energy-manager.ts`) on whichever system's draw the reactor couldn't fully cover (under shortage every draw gets the same supply ratio), cleared the moment that same draw is covered again. The `Reactor` itself is the one exception: `EnergyManager.update()` sets `reactor.energyStarved` directly whenever `reactor.energy` reaches zero, since a reactor with nothing left to give may have nothing currently *drawing* from it either — without this it would read as fully healthy on the Full Systems Status panel.

`RepairProtocolSlot.energyStarved` (`repair-queue.ts`) is a distinct field on the same concept for repair protocols specifically: true for every tick a RUNNING slot's declared energy draw isn't fully covered. A partial grant advances the run (progress and heat) at that share; a tick with nothing granted stalls it, and an outage sustained past `ENERGY_STARVATION_GRACE_SECONDS` force-stops it (see `RepairManager.tickRunning`). It's what lets the repair-queue widget show *why* a progress bar is slow or stalled, before that slot's own `refusalReason` has anything to say.

### Repair Protocols (issue #2247, modes and cancel speed in #2255)

Each catalog protocol (`configurations/repair-protocols.ts`) has its own `RepairProtocolSlot` on `ShipState.repairQueue.slots` — one per protocol, in catalog order, created once and never resized. A slot's `priority` (`RepairPriority`: `OFF`/`LOW`/`MEDIUM`/`HIGH`/`RUNNING`/`CANCELLING`) is its whole scheduling state; a separate `mode` field (`RepairProtocolMode`: `Responsive`/`Dark`, see below) tracks a field-tier slot's chosen price. There is no separate queue list, so a protocol never needs to be in the backlog more than once.
...
- **Modes (field-tier only, issue #2255):** every field-tier protocol offers two modes, picked per slot via a dedicated `ctrl+alt+<n>` gesture (`toggleRepairProtocolMode`) independent of the priority ladder, and locked once the slot is `RUNNING`/`CANCELLING`. **Responsive** is today's catalog duration with no side effect — the system stays live while the repair runs. **Dark** is a third of that duration, with the historical side effect (the darkened system(s) forced to `PowerLevel.SHUTDOWN` for the run). `energyDraw` (a rate) and `heat` (a fixed total budget) are unchanged between modes: Dark's shorter duration means a third the total energy cost but the same total heat, delivered three times as fast. Docked/shipyard-tier protocols stay single-mode (flat `duration`/`energyDraw`/`heat`/`sideEffectSystems`, same shape as before #2255) — `getModeStats(protocol, mode)` is the one seam that reads either shape uniformly.
- **Cancel = wind-down, at 2x speed (issue #2255, R2):** either priority key on a `RUNNING` slot sets `CANCELLING`, and progress runs back to 0% (like missile unload) instead of stopping instantly, at `CANCEL_WINDDOWN_SPEED_MULTIPLIER` (2x) forward speed — cancelling at progress `p` costs `p / 2` instead of `p`, moving the break-even point to 2/3 progress. A key pressed while `CANCELLING` is ignored. At 0%, side effects revert and a spent energy cell (see below) is refunded.
- **Completion:** at 100%, the protocol's effect applies and the slot returns to `OFF` — no auto-repeat.
- **Availability:** a slot that can't run now (tier, energy cell, missing equipment) refuses the priority raise with a per-slot `refusalReason`, cleared the next time the player changes that slot's priority. A pending or running slot that loses availability (undocking, sustained energy starvation) is force-stopped to `OFF` with a reason, refunding a spent energy cell the same as a completed cancel wind-down (see below).
- **Energy cells:** `consumesEnergyCell` protocols (e.g. `reactorJumpStart`) spend one `Reactor.energyCells` the instant they start `RUNNING`, not on completion — same timing as chain-gun ammo, which leaves the magazine at load start. There is no reservation accounting: a protocol can't be pending without a cell being available at that moment. The cell stays spent only on a successful completion; every other exit from `RUNNING`/`CANCELLING` (a cancel wind-down reaching 0%, a forced tier-loss/energy-starvation stop, or a stranded slot found by `resetShipState` across an NPC↔PC conversion) refunds it, capped at `design.maxEnergyCells` — see `revertRepairSlot` in `repair-manager.ts`.
- **Display filtering:** the repair-queue widget hides a protocol row only when this ship structurally lacks the equipment it targets (fixed per ship design, checked once at draw time via `hasProtocolEquipment`) — a docked-tier or out-of-cells protocol stays visible and explains itself through the slot's `refusalReason` instead.
- **Input:** the engineer screen is hotkeys only (`alt+<n>` raises, `alt+shift+<n>` lowers, `ctrl+alt+<n>` toggles mode on a field-tier row — no buttons); the GM screen keeps click controls (raise/lower/mode-toggle buttons per protocol, issue #2212).
- **Coverage (issue #2255, R3):** `findUncoveredDefectibleFields` walks every `@defectible` field on every `REPAIRABLE_SYSTEM_KEYS` system and reports any not targeted by some catalog protocol — a test in `repair-protocols.spec.ts` pins this at zero, so a new defectible added to a repairable system without a matching protocol fails CI instead of silently decaying with no counterplay (the `radars/turnSpeedFactor` gap issue #2109 found).
