...
## 2026-05-03 — Scan tiers: passive read + EW beam

**Tier 1 — passive read.** Passive identification of a contact held on radar. Reveals **faction, ship class, callsign**. Automatic on all radars (Pilot, Weapons, GM, Signals); no Signals action required.
- **Promotion takes 5 seconds.** A contact enters radar range as `ScanLevel.UFO` (0). After **5 seconds of continuous time in radar range** it auto-promotes to `ScanLevel.BASIC` (1). The delay adds suspense — the bridge sees an unknown contact for several seconds before identification.
