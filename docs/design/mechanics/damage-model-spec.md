...
> **Status: design document.** This is the agreed target design for the armor × ammo damage
...
a property of the warhead — the ClusterMissile switches type with its selected mode.

**Terms:**

- **Damage type** — the family a round belongs to: what kind of harm it does.
- **Damage profile** — the per-type behavior bundle defined in section 3–5 (scope, layer,
...
- **Damage event** — one application of damage to one ship: an amount + a surface arc + a
...
- **Impact** — the round must **physically hit the ship** (contact fuze; no proximity
  ...
  contact point: narrow arc → one armor plate, one ship area. The event's amount is the round's
  **damage** number.
- **Explosion** — the round detonates into a blast object that grows over time. Every game tick
  ...
  `damageFactor × tickSeconds × overlap`. Big, lingering blasts therefore deliver **many small
  events**; wide blasts can cover both ship areas and several ships at once.
- **Blast size** — the explosion's maximum radius = `expansionSpeed × secondsToLive`. Its
...
fixed-size malfunction specific to that system (an aim offset, a capacity loss…). Enough
defects eventually make a system `broken`. How many rolls a target gets, who the target is, and
how often a system can be hurt are the three mechanics below.

**Terms:**

- **Defect roll** — a probability check: chance = `cdf(event amount × factors)` against the
  system's `damage50` (the amount at which a roll is a coin flip). Strong hits ≈ certain,
  weak hits ≈ rare. One success = one defect, always the same size — a stronger hit raises the
...
- **Concentration** — **how many defect rolls the target receives per damage event.** A
  ...
  concentration = the round dumps its energy into its victim: an 8-concentration hit gives its
  victim 8 rolls at once.
...
- **Defect cooldown** — **explosion delivery only**: a system accepts at most one damage-event
  ...
  per round; its rate limit is the weapon's rate of fire.
...
- **Surface channel (scrape)** — HiExp and Frag damage hull-mounted systems in the arc on
  **every hit, regardless of armor**: the equipment sits outside the plates. Strength =
  `amount × 0.05 × surfaceDamageFactor` (Frag 2 — purpose-built shredder; HiExp 0.25 — a wash).
- **Deflectable** — a deflecting armor (Reactive) pushes the round away before its blast
  ...
  defeats the deflection) **and Frag** (a cloud is not a round — nothing to push away).
- **System damage factor** — multiplier on the penetrating channel's roll amounts (e.g. ArmPen
  ×1.5 concentrated punch, Elec ×2).

| Type | Scope | Layer | Factor | Scrape | Deflectable |
| --- | --- | --- | --- | --- | --- |
| HiExp | multi | internal | 1 | yes ×0.25 | yes |
| ArmPen | single | internal | 1.5 | no | yes |
| Frag | multi | external | 0.5 | yes ×2 | **no** |
...
- **`plateDamage`** — multiplier on **plate erosion only** (never on system damage — armor
  ...
  all. Scale: 0 immune · 0.25/0.5 resistant · 1 normal · 2 vulnerable; anything between is
  legal, per model, data-only.
...
- **Exposure** — how open a hit arc is: `max(penetration, brokenPlateRatio)`. Scales the
  penetrating channel's roll amounts. Broken plates = the hole damage flows through.
...
| --- | --- | --- | --- |
| Composite | the baseline hull layer | ArmPen (2×) | no walls, no gaps |
| Whipple | standoff screen | ArmPen ignores it (0/1) | blunts blast, pre-detonates shaped charges |
| Hardened | thick slab | Tandem jet (2×) | the only armor that stops kinetic rounds |
...
**General explanation:** systems are classified by mounting (**internal**/**external** — decides
scrape eligibility and which layer penetrating damage reaches) and by nature
(**electronics** — decides Elec targeting). Classification is per system class today; per-ship
mounting is future work ([#1954](https://github.com/starwards/starwards/issues/1954)). Each
defect applies that system's own small malfunction.

| System | Mounting | Electronics | One defect does |
| --- | --- | --- | --- |
| Thrusters | external | no | angle error ±1–3° or capacity −0.01..0.1 |
| Chain gun / tubes | external | yes | aim offset ±1–2° or rate-of-fire ×0.9 |
| Radar | external | yes | malfunction range +5% |
| Docking | external | yes | ranges −5% |
| Signals | external | yes | job speed −5% or success −5% |
| Reactor | internal | yes | energy ×0.9 or efficiency −5% |
| Magazine | internal | yes | lose ~10% of one ammo stock or capacity ×0.9 |
| Warp | internal | yes | damage factor +0.05 or velocity ×0.9 |
| Maneuvering | internal | no | efficiency −5% or afterburner fuel ×0.9 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HiExpShell | HiExp | explosion | multi | — | 1 | 20/tick | 200m × 1s |
| ArmPenShell | ArmPen | impact | single | per shell | **1** | 30 | — |
| FragShell | Frag | explosion | multi ext. | — | 1 | 10/tick | 250m × 1s |
| HiExpMissile | HiExp | explosion | multi | — | 1 | 50/tick | 350m × 0.35s |
| ArmPenMissile | ArmPen | impact | single | yes | **8** | 60 | — |
| FragMissile | Frag | explosion | multi ext. | — | 1 | 10/tick | 800m × 1.6s |
| Cluster — Frag mode | Frag | explosion | multi ext. | — | 1 | 10/tick | 750m × 1s |
| Cluster — AP mode | ArmPen | impact | **multi** | — | **3** | 30 | — |
| TandemMissile | Tandem | impact | single | yes | **5** | 50 | — |
| ElecMissile | Elec | impact | electronics | — | 1 | 25 | — |
...
**shell** is concentration 1 on purpose: its power is anti-armor (plate erosion at 2× vs
...
**Missile flight table** (all: proximity detonation 100m for explosion delivery; impact
missiles are contact-fuzed and can be dodged):

| Missile | Max speed | Turn rate | Flight time | Fuze |
| --- | --- | --- | --- | --- |
| HiExpMissile | 600 | 720°/s | 78s | proximity |
| ArmPenMissile | 960 | 504°/s | 42s | **contact** |
| FragMissile | 600 | 720°/s | 78s | proximity |
| ClusterMissile | 600 | 720°/s | 78s | by mode: Frag proximity / AP **contact** |
| TandemMissile | 420 | 936°/s | 60s | **contact** |
| ElecMissile | 780 | 720°/s | 96s | **contact** |
...
struck plate erodes 120; exposure 0 (plates intact, no penetration) → **no system damage**.
Armor did its job; repeat hits break the plate, then the assassin gets in.

**ArmPen missile vs breached Composite section:** exposure 1 → sticky victim picked → 8 rolls ×
`cdf(60 × 1.5)` ≈ certain → **~8 defects into one system** — crippled or broken. Everything
else in the section: untouched.

**HiExp missile vs the same breach:** ~21 explosion events over 0.35s, cooldown rations to
~2–3 applications per system → **every internal in the section takes ~2–3 defects**, plus the
externals take the (weak, ×0.25) scrape. Whole section bleeds; ECR triage.

**Frag missile vs anyone:** armor row 0/0 everywhere — plates never touched, ERA never wakes.
The 800m cloud lingers 1.6s; every external in the arc takes rationed scrape rolls at ×2 —
...
**electronics safe, one cell spent**. Same missile vs Composite: 0/1 transparent → every
electronics system ship-wide rolls once at ×2.

## 10. Tuning knobs

| Knob | Where | Governs |
| --- | --- | --- |
| armor rows (`plateDamage_*`, `penetration_*`, flags) | `configurations/armor-models.ts` | every armor × type matchup |
