...
**Anti-Abstraction Game Design** - Rejection of hit points and damage points. Prefer concrete system states over opaque health metrics. Let players create their own abstractions from technical data (e.g., engineers summarizing "core at eighty percent" while pilots see raw system data). This builds role niches.

**UI: Separate Information From Interaction** - Screens should display information with minimal interactive elements on screen. Prefer joysticks and keyboards over touchscreens and mouse. Drawing from Bret Victor's "Interaction is an essentially negative aspect of information software" philosophy.
...
- Won't damage large armored ships, but deadly to unarmored fighters or torpedoes
